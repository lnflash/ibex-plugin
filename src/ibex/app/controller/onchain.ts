...
async function createBitcoinAddressHandler(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    const response: AxiosResponse<{ address: string }> = await axios.post(
      `${IBEXEnum.BASE_URL}onchain/address`,
      { accountId, webhookUrl, webhookSecret },
      {
...
async function sendStatsToBitcoinHandler(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    // const feeSat = await estimateFee(token, address, amount);
    const response: AxiosResponse<IbexOnchainTransaction> = await axios.post(
      `${IBEXEnum.BASE_URL}v2/onchain/send`,
      { accountId, address, amount, webhookUrl, webhookSecret },
      {
        headers: { Authorization: token },
      }
    );
...
async function getTransactionDetails(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    const response: AxiosResponse<IbexOnchainTransaction> = await axios.get(`${IBEXEnum.BASE_URL}v2/transaction/${transactionId}`, {
      headers: { Authorization: token },
    });
    res.status(200).json({ data: response.data });
  } catch (error: any) {
