...
async function payInvoiceHandler(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    if (
      !(
        (typeof accountId === "string" && typeof bolt11 === "string") ||
    ...
    const paymentRes: AxiosResponse<IbexPayInvoice> = await axios.post(
      `${IBEXEnum.BASE_URL}v2/invoice/pay`,
      { accountId, bolt11, amount, webhookUrl, webhookSecret },
      {
        headers: { Authorization: token },
      }
    );
