...
} from "@ibex/app/model/lightninginvoice";
...
async function createInvoiceHandler(req: Request, res: Response): Promise<void> {
  ...
  try {
    const response: AxiosResponse<IBEXLightningInvoice> = await axios.post(
      `${IBEXEnum.BASE_URL}v2/invoice/add`,
      { amount, accountId, memo, expiration, webhookUrl, webhookSecret },
      {
