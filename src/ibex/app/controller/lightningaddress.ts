...
} from "@ibex/app/model/lightningaddress";
...
async function createLightningAddressHandler(req: Request, res: Response): Promise<void> {
  try {
    ...
    const response: AxiosResponse<IBEXLightningAddressData> = await axios.post(
      `${IBEXEnum.BASE_URL}lightning-address`,
      { username, accountId, webhookUrl, webhookSecret },
      {
...
export {
  createLightningAddressHandler,
  getLightningAddressesByAccountIdHandler,
  updateLightningAddressHandler,
  deleteLightningAddressHandler,
