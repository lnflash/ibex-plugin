...
async function createLNURLHandler(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    const response: AxiosResponse<{ lnurl: string }> = await axios.post(
      `${IBEXEnum.BASE_URL}lnurl/pay`,
      { accountId, currencyId, amount, webhookUrl, webhookSecret },
      {
...
async function invoiceRequirementHandler(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    const response: AxiosResponse<{ callback: string; maxSendable: number; minSendable: number; metadata: string; tag: string }> = await axios.get(
      `${IBEXEnum.BASE_URL}lnurl/pay/invoice-requirements?k1=${k1}`,
      {
        headers: { Authorization: token },
      }
    );
...
async function paylnurlHandler(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    const response: AxiosResponse<{ lnurl: string }> = await axios.post(
      `${IBEXEnum.BASE_URL}v2/lnurl/pay/send`,
