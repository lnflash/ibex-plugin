...
async function createLightningInvoice(data: IBEXLightningInvoice): Promise<string[]> {
    await createTable();
    return db(TABLE_NAME).insert({ ...data, totalFee: data.networkFee + data.onChainSendFee });
}

async function getLightningInvoiceByBoult11(boultll: string): Promise<Pick<IBEXLightningInvoice, 'invoice'> | undefined> {
    return db(TABLE_NAME).select('invoice')
        .whereRaw(`invoice->>'bolt11'='${boultll}'`).first()

}
async function updateLightningInvoiceByBoult11(boultll: string, body?: Invoice, settledAt?: string, fee?: number): Promise<Invoice | undefined> {
    if (settledAt && body) {
...
async function deleteLightningInvoice(boultll: string): Promise<number> {
    return db(TABLE_NAME).delete().whereRaw(`invoice->>'bolt11'='${boultll}'`);
}


export { IBEXLightningInvoice, IBEXLightningInvoiceWebhookPayload, Invoice, InvoiceState, createLightningInvoice, getLightningInvoiceByBoult11, updateLightningInvoiceByBoult11, deleteLightningInvoice };
